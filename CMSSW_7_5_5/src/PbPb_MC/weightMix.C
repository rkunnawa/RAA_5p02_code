...
void weightMix(){
  ...
  nt = new TChain("akVs4PFJetAnalyzer/t");
  //nt->AddFriend(evt);
  ...
  for(int i = 0; i<Nfiles; ++i){
    ...
    TTree * newTree = (TTree*)fin->Get("akVs4PFJetAnalyzer/t");
    ...
    outf[i] = new TFile(Form("weights_pbpb_5p02TeV_PYHYD_%2.0f.root",pthatBins2015[i]),"recreate");
